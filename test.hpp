...
BOOST_AUTO_TEST_CASE( Second_test ) ///< Sprawdzamy  dodwanie poprzecznych linii
{
    ...
    Menu::open_file( c,  arg);
    converter conv3;
    conv3.load(Menu::temp);
    conv3.translate();
    BOOST_CHECK_EQUAL("<UL>", conv3.get_output(7));
    BOOST_CHECK_EQUAL("<LI>aaa</LI>", conv3.get_output(8));
    BOOST_CHECK_EQUAL("<LI>bbb</LI>", conv3.get_output(9));
    BOOST_CHECK_EQUAL("<LI>ccc</LI>", conv3.get_output(10));
    BOOST_CHECK_EQUAL("<LI>dddd</LI>", conv3.get_output(11));
    BOOST_CHECK_EQUAL("<LI>dd<EM>ddd</EM>+<STRONG>ala</STRONG></LI>", conv3.get_output(12));
    BOOST_CHECK_EQUAL("</UL>", conv3.get_output(13));
    BOOST_CHECK_EQUAL("<HR>", conv3.get_output(15));
    BOOST_CHECK_EQUAL("<HR>", conv3.get_output(16));
    free (arg[0]);
    free (arg[1]);
    free (arg[2]);
    Menu::temp.clear();
